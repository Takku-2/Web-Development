  ...
  $("#update_btn").click(function (e) {
    ...
    element_li.each((index, element) => {
      if (element.innerHTML == update) {
        input_value = $("#" + $(element).attr("value"))
          .find("#" + $(element).attr("value") + "1")
          .val();
        if (!input_value) {
          alert("Please Enter Value For " + update + " To Update");
        } else if (
          $(element).attr("value") === "Flat_no" &&
          input_value.length != 4
        ) {
          alert("Please Enter Valid Flat Number");
        } else if (
          $(element).attr("value") === "phone_no" &&
          input_value.length != 10
        ) {
                ...
                $.ajax({
                  ...
                  success: function (response) {
                    if (response.msg === "success") {
                      alert("Updation Of Details Is SuccessFull");
                      window.location.replace(
                        "http://localhost:3000/users/members"
                      );
                    } else if ((response.msg = "error")) {
                      alert("Some Error Occured Try Again!");
                    }
